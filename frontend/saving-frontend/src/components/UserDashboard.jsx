...
export function UserDashboard() {
  ...
  const { data: depositsData, isLoading } = useReadContracts({
  ...
  // 5. Fetch current accrued interest for each NFT
  const { data: interestData } = useReadContracts({
    contracts: userIds.map((id) => ({
      address: SAVING_CORE_ADDRESS,
      abi: SAVING_CORE_ABI,
      ...
      {userIds.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {depositsData?.map((result, index) => {
            ...
            const interest = interestData?.[index]?.result || 0n;
